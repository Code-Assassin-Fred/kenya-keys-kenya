...
import { Mail, Lock, User, Loader2, ArrowLeft, Eye, EyeOff, CheckCircle, RefreshCw } from 'lucide-react';
import Link from 'next/link';
import { useRouter } from 'next/navigation';
import Image from 'next/image';
import { auth } from '@/lib/firebase';
import { GoogleAuthProvider, signInWithPopup, createUserWithEmailAndPassword, sendEmailVerification, signOut } from 'firebase/auth';
...
export default function AdminSignupPage() {
    ...
    async function handleSignup(e: React.FormEvent<HTMLFormElement>) {
        ...
        try {
            ...
            // 2. Trigger native Firebase verification email
            await sendEmailVerification(user);

            // 3. Get User ID token to verify on backend and setup Firestore user document
            ...
            if (res.ok && result?.success) {
                ...
                setVerifiedEmail(data.email as string);
                setCooldown(60);
            } else {
                setError(result?.error || "Signup failed.");
                // Clean up newly created client auth user on failure
    ...
    async function handleResendEmail() {
        ...
        try {
            ...
            if (user) {
                ...
                setResendError('Authentication session not found. Please try to log in.');
            }
        } catch (err: any) {
    ...
    const maskedEmail = verifiedEmail ? (() => {
                ...
                <div className="absolute bottom-12 left-10 right-10 z-10">
                    ...
                    >
                        ...
                        <h2 className="text-white text-2xl font-black uppercase tracking-tight leading-tight font-outfit">
                            {verificationSent ? 'Almost There' : 'Join Our Mission'}
                        </h2>
                        <p className="text-white/70 text-sm mt-3 max-w-lg leading-relaxed font-medium font-outfit">
                            {verificationSent 
                                ? 'One more step to secure your admin account.'
                                : 'Create your admin account to help manage education programs across Kenya.'
                            }
                        </p>
                    </motion.div>
                </div>
                ...
                >
                    ...
                    {verificationSent ? (
                        ...
                        >
                            <Link
                                href="/admin/login"
                                className="flex items-center gap-2 text-[#3B82F6] font-bold uppercase tracking-wider text-xs mb-6 hover:translate-x-[-4px] transition-all no-underline font-outfit"
                            >
                                <ArrowLeft size={14} />
                                Back to Login
                            </Link>
                            ...
                            <motion.div
                                ...
                                className="w-20 h-20 bg-gradient-to-br from-[#3B82F6] to-[#1D366D] rounded-2xl flex items-center justify-center mx-auto mb-6 shadow-lg shadow-blue-500/20"
                            >
                                <Mail className="w-10 h-10 text-white" />
                            </motion.div>

                            <h1 className="text-xl font-bold text-[#101828] font-outfit uppercase tracking-wide text-center">
                                Check Your <span className="text-[#3B82F6]">Email</span>
                            </h1>
                            <p className="text-[#667085] mt-3 text-sm font-medium font-outfit text-center leading-relaxed">
                                We&apos;ve sent a verification link to
                            </p>
                            {maskedEmail && (
                                <p className="text-[#101828] font-bold font-outfit text-sm text-center mt-1">
                                    {maskedEmail}
                                </p>
                            )}
                            <p className="text-[#667085] mt-3 text-xs font-medium font-outfit text-center leading-relaxed">
                            ...
                            {resendMessage && (
                                <motion.div
                                    ...
                                    animate={{ opacity: 1, y: 0 }}
                                    className="bg-emerald-50 text-emerald-700 p-3.5 rounded-xl text-xs font-semibold font-outfit border border-emerald-100 mt-5 flex items-center gap-2"
                                >
                            ...
                            {resendError && (
                                <motion.div
                                    initial={{ opacity: 0, y: -10 }}
                                    animate={{ opacity: 1, y: 0 }}
                                    className="bg-red-50 text-red-600 p-3.5 rounded-xl text-xs font-semibold font-outfit border border-red-100 mt-5 flex items-center gap-2"
                                >
                            ...
                            <div className="mt-8 space-y-3">
                                <button
                                    ...
                                    className="w-full py-3.5 rounded-xl bg-[#1D366D] text-white font-bold font-outfit uppercase tracking-wider text-sm hover:bg-[#101828] hover:shadow-lg transition-all flex items-center justify-center gap-3 disabled:opacity-50 disabled:cursor-not-allowed"
                                >
                                    {isResending ? (
                                ...
                                <Link
                                    href="/admin/login"
                                    className="block w-full py-3.5 rounded-xl bg-white border-2 border-[#E2E8F0] text-[#344054] font-bold font-outfit uppercase tracking-wider text-sm hover:border-[#3B82F6] hover:shadow-md transition-all text-center no-underline"
                                >
                            ...
                            <div className="mt-8 p-4 bg-[#F8FAFC] rounded-xl border border-[#E2E8F0]">
                                <p className="text-xs text-[#667085] font-outfit font-medium leading-relaxed">
                                    <strong className="text-[#344054]">Didn&apos;t receive the email?</strong>
                                    <br />
                        ...
                        <>
                            ...
                            <div className="mb-6">
                                <Link
                                    href="/admin/login"
                                    className="flex items-center gap-2 text-[#3B82F6] font-bold uppercase tracking-wider text-xs mb-4 hover:translate-x-[-4px] transition-all no-underline font-outfit"
                                >
                                    <ArrowLeft size={14} />
                                    Back to Login
                                </Link>
                                <h1 className="text-xl font-bold text-[#101828] font-outfit uppercase tracking-wide">
                                    Create <span className="text-[#3B82F6]">Account</span>
                                </h1>
                            ...
                            <p className="text-center text-[#667085] text-sm mt-6 font-outfit">
                                ...
                                <Link href="/admin/login" className="text-[#3B82F6] font-bold hover:text-[#1D366D] transition-all no-underline">
                                    Log In
                                </Link>
                            </p>
                        </>
